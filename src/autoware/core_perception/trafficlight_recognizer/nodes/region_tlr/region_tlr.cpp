...
static void image_raw_cb(const sensor_msgs::Image& image_source)
{
  ...
  for (unsigned int i = 0; i < detector.contexts.size(); i++)
  {
    ...
    circle(targetScope, detector.contexts.at(i).redCenter, detector.contexts.at(i).lampRadius, cvScalar(255, 0, 0), 1, 0);
    circle(targetScope, detector.contexts.at(i).yellowCenter, detector.contexts.at(i).lampRadius, cvScalar(255, 255, 0),
           1, 0);
    circle(targetScope, detector.contexts.at(i).greenCenter, detector.contexts.at(i).lampRadius, cvScalar(0, 255, 0), 1,
           0);
  }
